...
existing_subsets = ['MC3D-clusters', 'MC3D', 'MC3D-random', 'MC3D-rattled',  'MC2D', 'MC3D-surfaces', 'SHIFTML-molcrys', 'SHIFTML-molfrags']


def get_triple_row_plot(datasets_1, datasets_8, labels, cbar_title="cohesive energy (eV/atom)", save_path=""):
    ...
    for i, (pcovr, energy, title) in enumerate(datasets_1):
        ...
        if i == N_COLS - 1:
            ...
            ax_right.tick_params(which='both', bottom=False, left=False, top=False, right=False, 
    ...
    for i, (pcovr, energy, title) in enumerate(datasets_8):
        ...
        if i == N_COLS - 1:
            ax_right = ax.twinx()
            ax_right.set_ylabel("Cumulant features", rotation=-90, labelpad=35, fontsize=20)
            ax_right.tick_params(which='both', bottom=False, left=False, top=False, right=False, 
                               labelbottom=False, labelleft=False, labeltop=False, labelright=False)
    ...
    for i, (pcovr, energy, title) in enumerate(datasets_8):
        ...
        if i == N_COLS - 1:
            ax_right = ax.twinx()
            ax_right.set_ylabel("Cumulant features\n by cohesive energy", rotation=-90, labelpad=45, fontsize=20)
            ax_right.tick_params(which='both', bottom=False, left=False, top=False, right=False, 
                               labelbottom=False, labelleft=False, labeltop=False, labelright=False)
...
def plot_pcovr_lips(datasets, labels_lips, save_path=""):
    ...
    gs = gridspec.GridSpec(2, 3, height_ratios=[1, 1], width_ratios=[1, 1, 1], wspace=0.04, hspace=0.04)
    ...
    subset_style = {
        ...
        "surface_hydrogen": {"label": "Surface hydrogen", "color": "#00FF00", "marker": "P"},
    }
    ...
    for i, ax in enumerate(axes):
        if i < N_MODELS:
            ...
            ax.scatter(
                mad_features[:, 0], mad_features[:, 1],
                color="grey", alpha=0.2, s=65, 
                label="MAD test", edgecolors="none", rasterized=True
            )

            for subset, style in subset_style.items():
                mask = np.array(labels_lips) == subset
                ax.scatter(
                    lips_features[mask, 0], lips_features[mask, 1],
                    color=style["color"], marker=style["marker"], s=65,
                    alpha=0.9, edgecolors="black", linewidth=0.3,
                    label=style["label"], rasterized=True
                )
